...
class SearchLogsRequest(BaseModel):
    ...
    time_range: Optional[str] = Field(None, description="Time range (e.g., '1h', '24h', '7d'). Defaults to '1h' if not specified.")
...
class AggregationRequest(BaseModel):
    ...
    aggregation_type: str = Field(..., description="Aggregation type (terms, date_histogram, etc.)")
...
class StreamSearchRequest(BaseModel):
    ...
    stream_id: str = Field(..., description="Stream ID (e.g., '5abb3f2f7bb9fd00011595fe' for 1c_eventlog)")
    query: str = Field(..., description="Search query (e.g., '*' for all messages, 'level:ERROR' for errors)")
    time_range: Optional[str] = Field(None, description="Time range (e.g., '1h', '24h', '7d'). Defaults to '1h' if not specified.")
    fields: Optional[List[str]] = Field(None, description="Fields to return (e.g., ['message', 'level', 'source'])")
...
def test_connection() -> str:
    ...
    try:
        ...
        return json.dumps({
            ...
            "endpoint": config.graylog.endpoint
        }, indent=2)
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting MCP Graylog server on {config.server.host}:{config.server.port}")
