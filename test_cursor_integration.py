...
def test_docker_image():
    ...
    try:
        result = subprocess.run(
            ['docker', 'images', 'mcp-graylog:latest'],
            capture_output=True,
            text=True
        )
...
def test_container_run():
    ...
    try:
        ...
        cmd = ['docker', 'run', '--rm', '--name', container_name] + env_vars + ['mcp-graylog:latest', 'timeout', '10', 'python', '-m', 'mcp_graylog.server']
        ...
        "mcpServers": {
            "graylog": {
                ...
                "args": [
                    ...
                    "--rm", 
                    "-i", 
                    "-e", f"GRAYLOG_ENDPOINT={endpoint}",
                    "-e", f"GRAYLOG_USERNAME={username}",
                    "-e", f"GRAYLOG_PASSWORD={password}",
